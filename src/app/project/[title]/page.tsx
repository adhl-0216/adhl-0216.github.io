...
async function preprocessMarkdown(markdown: string): Promise<string> {
    const fallbackImageUrl = '/apps.svg';
    const imageRegex = /!\[(.*?)\]\((.*?)\)/g;
    let match;
    ...
    while ((match = imageRegex.exec(markdown)) !== null) {
        ...
        if (!isValid) {
            // processedMarkdown = processedMarkdown.replace(fullMatch, `![${altText}](${fallbackImageUrl})`);
